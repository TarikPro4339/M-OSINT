...
import sys, os, time, threading, re, socket, hashlib, ssl, json, codecs, ipaddress, math, secrets
...
def dns_hardening_check(domain: str) -> dict:
    """CAA ve DNSSEC sinyallerini pasif DNS sorgularıyla özetler."""
    domain = _clean_domain(domain)
    r = {"domain": domain, "caa": [], "dnskey_found": False, "ds_found": False,
         "score": 0, "findings": [], "error": None}
    if not domain:
        r["error"] = "Geçerli bir alan adı girin"
        return r
    def query(name, record):
        try:
            return [str(x) for x in dns.resolver.resolve(name, record, lifetime=5)]
        except (dns.resolver.NoAnswer, dns.resolver.NXDOMAIN, dns.resolver.NoNameservers):
            return []
        except Exception:
            return []
    r["caa"] = query(domain, "CAA")
    r["dnskey_found"] = bool(query(domain, "DNSKEY"))
    r["ds_found"] = bool(query(domain, "DS"))
    if r["caa"]:
        r["score"] += 40
        r["findings"].append("CAA kaydı bulundu; sertifika yetkilileri sınırlandırılmış olabilir.")
    else:
        r["findings"].append("CAA kaydı bulunamadı.")
    if r["dnskey_found"]:
        r["score"] += 30
        r["findings"].append("DNSKEY kaydı bulundu.")
    if r["ds_found"]:
        r["score"] += 30
        r["findings"].append("DS kaydı bulundu; üst bölge delegasyonu mevcut olabilir.")
    if not (r["dnskey_found"] and r["ds_found"]):
        r["findings"].append("DNSSEC durumu doğrulanmış değildir; kayıtların varlığı tek başına doğrulama anlamına gelmez.")
    return r

def ioc_normalize(text: str) -> dict:
    """Defanged IOC metnini ağ isteği yapmadan standart biçime getirir."""
    text = (text or "")[:50000]
    if not text.strip():
        return {"error": "Normalleştirilecek metin girin"}
    normalized = re.sub(r"(?i)hxxps(?=://)", "https", text)
    normalized = re.sub(r"(?i)hxxp(?=://)", "http", normalized)
    normalized = re.sub(r"\[\.\]|\(\.\)|\{\.\}", ".", normalized)
    normalized = re.sub(r"\[@\]|\(@\)|\{@\}", "@", normalized)
    extracted = ioc_extract(normalized)
    extracted["normalized_text"] = normalized
    extracted["note"] = "Normalleştirme yerelde yapıldı; hiçbir ağ isteği gönderilmedi."
    return extracted

def domain_registration_check(target: str) -> dict:
    """WHOIS bilgisinden alan adı kayıt özetini üretir."""
    domain = _clean_domain(target)
    r = {"domain": domain, "registrar": None, "created": None, "updated": None,
         "expires": None, "age_days": None, "name_servers": [], "error": None}
    if not domain:
        r["error"] = "Geçerli bir alan adı girin"
        return r
    ...
    try:
        ...
        r["registrar"] = str(record.get("registrar") or "") or None
        r["created"] = date_value(record.get("creation_date"))
        r["updated"] = date_value(record.get("updated_date"))
        r["expires"] = date_value(record.get("expiration_date"))
        servers = record.get("name_servers") or []
        if isinstance(servers, str):
            servers = [servers]
        r["name_servers"] = sorted({str(s).lower().rstrip(".") for s in servers})[:20]
        created = record.get("creation_date")
        if isinstance(created, list):
            created = created[0] if created else None
        if isinstance(created, datetime):
            now = datetime.now(created.tzinfo) if created.tzinfo else datetime.now()
            r["age_days"] = max(0, (now - created).days)
    except Exception as e:
        r["error"] = f"WHOIS sorgusu başarısız: {str(e)[:160]}"
    return r

def email_policy_audit(domain: str) -> dict:
    """SPF ve DMARC kayıtlarındaki sahteciliğe karşı politika sinyallerini yorumlar."""
    base = email_auth_check(domain)
    if base.get("error"):
        return base
    spf = base.get("spf") or ""
    dmarc = base.get("dmarc") or ""
    def tag(value, key):
        match = re.search(rf"(?:^|;)\s*{key}\s*=\s*([^;\s]+)", value, re.I)
        return match.group(1).lower() if match else None
    spf_end = next((item for item in ("-all", "~all", "?all", "+all") if item in spf.lower().split()), None)
    policy = tag(dmarc, "p")
    subdomain_policy = tag(dmarc, "sp")
    pct = tag(dmarc, "pct") or "100"
    score = 0
    findings = []
    if spf:
        score += 20
        findings.append(f"SPF sonlandırıcısı: {spf_end or 'belirtilmemiş'}.")
        if spf_end == "-all": score += 20
        elif spf_end == "~all": score += 10
    else:
        findings.append("SPF kaydı bulunamadı.")
    if dmarc:
        score += 20
        findings.append(f"DMARC politikası: {policy or 'belirtilmemiş'}; kapsam: %{pct}.")
        if policy == "reject": score += 40
        elif policy == "quarantine": score += 25
        elif policy == "none": score += 5
    else:
        findings.append("DMARC kaydı bulunamadı.")
    return {"domain": base["domain"], "spf": spf or None, "dmarc": dmarc or None,
            "spf_terminal": spf_end, "dmarc_policy": policy,
            "subdomain_policy": subdomain_policy, "dmarc_percentage": pct,
            "score": min(score, 100), "findings": findings, "error": None}

def ioc_csv_export(text: str) -> dict:
    """Yerel IOC analiz sonucunu güvenli CSV metnine dönüştürür."""
    result = ioc_normalize(text)
    if result.get("error"):
        return result
    rows = ["type,value"]
    for kind in ("urls", "emails", "ips", "domains", "hashes"):
        for value in result.get(kind, []):
            safe = str(value).replace('"', '""')
            rows.append(f'{kind[:-1]},"{safe}"')
    return {"total": result.get("total", 0), "csv": "\n".join(rows),
            "note": "CSV yerelde üretildi; indirip bir dosyaya kaydedebilirsiniz."}
...
<div id="nav">
  ...
  <div class="nav-links">
    ...
    <div class="nl" onclick="goPage('tools',this)"><i class="fas fa-toolbox"></i>v0.0.7 Araçlar</div>
...
<div class="page active" id="page-dash">
  <div class="hero">
    ...
    <div class="vbadges">
      <span class="vbadge hot">⚡ v0.0.7 — NEON</span>
...
<div class="page" id="page-tools">
  <div class="th">
    <h2><i class="fas fa-toolbox" style="color:var(--neon)"></i>v0.0.7 Araç Kutusu <span class="newtag">10 ARAÇ</span></h2>
    <p>E-posta kimlik doğrulama, web ayak izi, security.txt, IOC analizi, DNS sertleştirme, WHOIS ve yerel şifre gücü analizi.</p>
    <div class="irow">
      <select class="msel" id="tool-mode" onchange="setToolHint()">
        ...
        <option value="ioc-normalize">Defanged IOC normalleştirici</option>
        <option value="dns-hardening">DNS sertleştirme: CAA / DNSSEC sinyalleri</option>
        <option value="domain-registration">Alan adı kayıt özeti: WHOIS / yaş</option>
        <option value="email-policy">E-posta sahteciliği politikası: SPF / DMARC</option>
        <option value="ioc-csv">IOC CSV dışa aktarma</option>
...
const toolConfig = {
  ...
  'ioc-normalize': {endpoint:'/api/ioc-normalize', field:'text', placeholder:'hxxps://ornek[.]com gibi defanged IOC metni yapıştırın...', hint:'Defanged URL, alan adı ve e-posta değerleri yerelde normalleştirilir; ağ isteği yapılmaz.'},
  'dns-hardening': {endpoint:'/api/dns-hardening', field:'domain', placeholder:'ornek.com', hint:'CAA ile DNSKEY/DS kayıtları pasif DNS sorgularıyla incelenir.'},
  'domain-registration': {endpoint:'/api/domain-registration', field:'target', placeholder:'ornek.com', hint:'WHOIS kaydından kayıt kuruluşu, tarihler, ad sunucuları ve alan adı yaşı özetlenir.'},
  'email-policy': {endpoint:'/api/email-policy', field:'domain', placeholder:'ornek.com', hint:'SPF sonlandırıcısı ile DMARC uygulama politikası yorumlanır.'},
  'ioc-csv': {endpoint:'/api/ioc-csv', field:'text', placeholder:'IOC veya defanged IOC metnini yapıştırın...', hint:'IOC değerleri yerelde normalleştirilir ve CSV metni oluşturulur.'},
...
async function doTool() {
  ...
  toast(result.error ? 'Araç uyarısı: ' + result.error : 'v0.0.7 araç sonucu hazır', result.error ? 'err' : 'ok');
}
...
function startRT() {
  ...
  if (log) log.innerHTML =
    `<span style="color:#2d3748">[ M-OSINT v0.0.7 — Gerçek Zamanlı ]</span><br><br>`;
...
# ══════════════════════════════════════════
app = Flask(__name__)
app.config['SECRET_KEY'] = os.environ.get('MOSINT_SECRET_KEY') or secrets.token_urlsafe(32)
...
@app.route('/api/ioc-normalize', methods=['POST'])
def api_ioc_normalize():
    d = request.get_json(silent=True) or {}
    return jsonify(ioc_normalize(d.get('text', '')))

@app.route('/api/dns-hardening', methods=['POST'])
def api_dns_hardening():
    d = request.get_json(silent=True) or {}
    return jsonify(dns_hardening_check(d.get('domain', '')))

@app.route('/api/domain-registration', methods=['POST'])
def api_domain_registration():
    d = request.get_json(silent=True) or {}
    return jsonify(domain_registration_check(d.get('target', '')))

@app.route('/api/email-policy', methods=['POST'])
def api_email_policy():
    d = request.get_json(silent=True) or {}
    return jsonify(email_policy_audit(d.get('domain', '')))

@app.route('/api/ioc-csv', methods=['POST'])
def api_ioc_csv():
    d = request.get_json(silent=True) or {}
    return jsonify(ioc_csv_export(d.get('text', '')))
