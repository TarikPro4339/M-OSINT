...
import sys, os, time, threading, re, socket, hashlib, ssl, json, codecs
...
def breach_password(password: str) -> dict:
    ...
    try:
        ...
        res = requests.get(
            ...
            headers={"User-Agent": "M-OSINT/0.0.3", "Add-Padding": "true"},
            timeout=10
        )
        if res.status_code == 200:
        ...
        try:
            ...
            if algo:
                res = requests.get(f"https://hashlookup.circl.lu/lookup/{algo}/{low}",
                    headers={"Accept": "application/json", "User-Agent": "M-OSINT/0.0.3"}, timeout=10)
                if res.status_code == 200:
                    try: d = res.json()
                    except: d = {}
...
def crtsh_lookup(domain: str) -> dict:
    ...
    try:
        ...
        for attempt in range(3):
            try:
                res = requests.get(url, headers={"User-Agent": "M-OSINT/0.0.3", "Accept": "application/json"}, timeout=25)
                if res.status_code == 200:
                    try:
                        data = res.json()
                        break
...
<div class="page active" id="page-dash">
  <div class="hero">
    ...
    <div class="vbadges">
      <span class="vbadge hot">⚡ v0.0.3 Alpha — NEON</span>
...
function startRT() {
  ...
  if (log) log.innerHTML =
    `<span style="color:#2d3748">[ M-OSINT v0.0.3 — Gerçek Zamanlı ]</span><br><br>`;
...
# ══════════════════════════════════════════
app = Flask(__name__)
app.config['SECRET_KEY'] = 'mosint-v003-secret'
